...
### Tests
- **`hookTimeout` now matches `testTimeout` at 30s, so a release can pass its own publish gate on a many-core box.** `vitest.config.ts` raised `testTimeout` to 30s because this real-database suite starves under parallel fork contention, but left `hookTimeout` at the vitest default of 10s. Setup hooks spawn more child processes than most tests (`tests/autolearn.test.ts`'s `beforeEach` forks 12 `git` calls plus a CLI `init`), so hooks starved first: three `prepublishOnly` runs on a 24-core Windows box failed with timeouts only, never an assertion, while every named file passed alone and CI stayed green.
